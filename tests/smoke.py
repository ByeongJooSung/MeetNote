...
async def run():
    ...
    async with async_playwright() as p:
        ...
        await pg.evaluate("__meetnote.cloudRows = [1, 2, 3].map(i => ({ id: 'm' + i, title: '회의 ' + i, fileId: 'F' + i, when: '2026-09-2' + i + 'T10:00', updatedAt: '2026-09-2' + i + 'T11:00:00Z' })).concat([{ id: 'old', title: '작년 회의', fileId: 'F0', when: '2025-03-01T10:00', updatedAt: '2025-03-01T11:00:00Z' }]); __meetnote.renderCloudList('db')")
        ...
        # 연도 필터: 처음엔 최근 연도(2026)만, 전체를 고르면 작년 것도
        expect(await pg.input_value('#cloudYear') == '2026' and await pg.locator('#cloudYear option').count() == 3, '목록 연도 필터: 회의록이 있는 연도만, 기본은 최근 연도')
        await pg.select_option('#cloudYear', '0'); await pg.wait_for_timeout(150)
        expect(await pg.locator('#cloudList .cl-item').count() == 4, '연도 "전체"를 고르면 모든 해의 회의록')
        await pg.select_option('#cloudYear', '2025'); await pg.wait_for_timeout(150)
        expect(await pg.locator('#cloudList .cl-item').count() == 1 and '작년 회의' in await pg.inner_text('#cloudList'), '연도 2025 선택 시 그 해만')
        await pg.click('[data-cview="cal"]'); await pg.wait_for_timeout(150)
        expect(await pg.locator('#cloudYear').is_hidden(), '달력 보기에서는 연도 필터 숨김')
        await pg.click('[data-cview="list"]'); await pg.wait_for_timeout(150)
        # 휴지통: 지운 회의록은 목록·건수에서 빠지고 휴지통 보기에서 복구·영구 삭제 버튼과 남은 날이 보인다
        await pg.evaluate("__meetnote.cloudRows = [1, 2, 3].map(i => ({ id: 'm' + i, title: '회의 ' + i, fileId: 'F' + i, when: '2026-09-2' + i + 'T10:00', updatedAt: '2026-09-2' + i + 'T11:00:00Z' })).concat([{ id: 'old', title: '작년 회의', fileId: 'F0', when: '2025-03-01T10:00', updatedAt: '2025-03-01T11:00:00Z' }, { id: 'del1', title: '지운 회의', fileId: 'FD', when: '2026-09-10T10:00', updatedAt: '2026-09-10T11:00:00Z', deletedAt: new Date(Date.now() - 5 * 86400000).toISOString(), deletedBy: 'me@example.com' }]); __meetnote.renderCloudList('db')")
        await pg.select_option('#cloudYear', '0'); await pg.wait_for_timeout(150)
        expect(await pg.locator('#cloudList .cl-item').count() == 4 and '지운 회의' not in await pg.inner_text('#cloudList') and await pg.inner_text('#dashTrashN') == '1', '휴지통에 든 회의록은 목록에서 빠지고 휴지통 건수 표시')
        await pg.click('[data-cview="trash"]'); await pg.wait_for_timeout(150)
        tx = await pg.inner_text('#cloudList')
        expect(await pg.locator('#cloudList .cl-item').count() == 1 and '지운 회의' in tx and '25일 뒤 자동 삭제' in tx and await pg.locator('[data-crestore]').count() == 1 and await pg.locator('[data-cpurge]').count() == 1, '휴지통 보기: 복구·영구 삭제 버튼과 남은 날')
        await pg.click('[data-cview="list"]'); await pg.wait_for_timeout(150)
        await pg.evaluate("__meetnote.cloudRows = []; __meetnote.renderCloudList('db')")
        # 반응형: 태블릿(820)은 왼쪽 메뉴 유지·카드 2열, 모바일(390)은 위쪽 띠 + "메뉴" 토글, 데스크톱은 250px 메뉴
        await pg.set_viewport_size({'width': 820, 'height': 1000}); await pg.wait_for_timeout(200)
        expect(await pg.evaluate("getComputedStyle(document.querySelector('#dashPage')).gridTemplateColumns.startsWith('200px') && getComputedStyle(document.querySelector('.dash-cards')).gridTemplateColumns.split(' ').length === 2 && getComputedStyle(document.querySelector('#lnbMore')).display === 'none'"), '태블릿: 좁은 왼쪽 메뉴·카드 2열·메뉴 토글 없음')
        await pg.set_viewport_size({'width': 390, 'height': 844}); await pg.wait_for_timeout(200)
        expect(await pg.evaluate("getComputedStyle(document.querySelector('#lnbMore')).display !== 'none' && getComputedStyle(document.querySelector('#cloudSync')).display === 'none' && getComputedStyle(document.querySelector('#dashProjs')).overflowX === 'auto'"), '모바일: 메뉴 토글 표시·도구 접힘·프로젝트 칩 가로 스크롤')
        await pg.click('#lnbMore'); await pg.wait_for_timeout(100)
        expect(await pg.evaluate("getComputedStyle(document.querySelector('#cloudSync')).display !== 'none'"), '모바일: 메뉴를 펼치면 도구 표시')
        await pg.click('#lnbMore'); await pg.wait_for_timeout(100)
        await pg.set_viewport_size({'width': 1400, 'height': 900}); await pg.wait_for_timeout(200)
        expect(await pg.evaluate("getComputedStyle(document.querySelector('#dashPage')).gridTemplateColumns.startsWith('250px')"), '데스크톱: 250px 왼쪽 메뉴')
