...
**자동으로 열기·되쓰기(선택)**: 구성원이 처음 열 때 "Drive 권한 허용"을 고르면 앱이 `drive`(전체) 권한을 추가로 요청하고(계정별로 기억), 그 뒤로는 선택창 없이 열리고 **고친 내용을 같은 파일에 되써** 올린 사람에게도 반영됩니다(`projects/{id}/meetings`에 `editedBy`·`editedAt` 기록). 선택창으로 연 파일도 그 파일에는 되쓸 수 있습니다. 되쓸 권한이 없으면 자기 Drive에 새 파일로 저장되고, 프로젝트 목록은 더 최근 기록을 따릅니다. 이 권한은 Google의 '제한된 범위'라 앱이 심사를 받지 않았으면 "확인되지 않은 앱" 화면이 뜨며(고급 → 이동), 테스트 사용자·소규모 팀에서만 쓸 수 있습니다. 허용하지 않으면 선택창 방식으로 동작합니다. 되돌리려면 설정 → 계정·클라우드 → Drive 연동 해제.
