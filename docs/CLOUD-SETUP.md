...
**자동으로 열기(선택)**: 구성원이 처음 열 때 "읽기 권한 허용"을 고르면 앱이 `drive.readonly`를 추가로 요청하고(계정별로 기억), 그 뒤로는 선택창 없이 열립니다. 이 권한은 Google의 '제한된 범위'라 앱이 심사를 받지 않았으면 "확인되지 않은 앱" 화면이 뜨며(고급 → 이동), 테스트 사용자·소규모 팀에서만 쓸 수 있습니다. 허용하지 않으면 선택창 방식으로 동작합니다. 되돌리려면 설정 → 계정·클라우드 → Drive 연동 해제.
