...
- **대시보드**(`#dashPage`, 전체 화면 페이지, 헤더 왼쪽 "← 대시보드" 버튼 / `dashOpen`·`dashClose`): 앱을 열면(로그인 시 `projPull` 뒤, 게스트도) 한 번 자동으로 열린다(`openDashboard`). 왼쪽 LNB에 프로젝트(모든 / 미분류(개인) / 프로젝트별, 건수·공유 표시)와 새 회의·새 프로젝트·프로젝트 관리·.mnote 열기·설정·Drive 맞추기·새로고침·"← 편집 화면으로"(이 세션에서 편집 화면에 들어간 뒤에만, `editorLive`)·로그아웃(`signOutAll`: 편집 내용을 비우고 계정에 묶인 프로젝트를 브라우저에서 지운 뒤 로그인 화면), 오른쪽에 현황 카드(회의록 수·이번 달·프로젝트·마지막 저장), 달력 카드(달력+그날 목록 또는 전체 목록 — 목록 보기에는 연도 필터 `#cloudYear`(회의록이 있는 연도만, 기본 최근 연도, 0=전체, `cloudUi.year`)), 최근 작성 회의록 카드(저장순 8건, `renderDashProjs`가 그린다). 목록은 내 `users/{uid}/meetings` + 공유 프로젝트의 `projects/{id}/meetings`를 합친다. **휴지통**(`cloudUi.view === 'trash'`, 달력·목록 옆 탭 `[data-cview=trash]`, 건수 `#dashTrashN`): 삭제는 `trashMark`로 내 기록과 프로젝트 기록에 `deletedAt`·`deletedBy`를 남기고 Drive 파일을 휴지통으로 옮긴다(`cloudDeleteItem`). `liveRows()`는 휴지통을 뺀 목록, `trashRows()`는 내가 지운 것. 복구 `cloudRestoreItem`, 영구 삭제 `cloudPurgeItem`(Drive delete + dbDelete + projMeetDelete). 30일(`TRASH_DAYS`) 지난 것은 대시보드를 열 때 `trashAutoPurge`가 정리한다(Drive 토큰이 있을 때만). 남이 지운 프로젝트 회의록은 목록에 보이지 않는다. **역할**: 마스터(`isProjOwner`) / 운영자(`p.inviters`, 최대 2명, UI 이름 '운영자') / 구성원. **운영 권한** `canModerate(m)`: 마스터·운영자(`isInviter`)는 구성원이 올린 행에도 삭제 버튼이 보이고, 휴지통·복구·영구 삭제를 프로젝트 기록(`projects/{id}/meetings`)에만 적용한다(남의 `users/{uid}/meetings`·Drive 파일은 못 건드림). 올린 사람 쪽은 `sharedRows`가 기록의 `deletedAt`을 자기 목록에 반영해 같은 회의록이 자기 휴지통에도 보인다. **프로젝트 알림**(`notifCheck`, 대시보드 `#dashBell`/`#notifDlg`): 서버 없이 `projects/{id}/meetings`의 `createdAt`(처음 올릴 때 기록)·`deletedAt`을 마지막 확인 시각(`meetnote.notifSeen.{uid}`, 프로젝트별)과 비교해 남이 올린 것·휴지통에 넣은 것을 모은다. 대시보드 로드 시와 3분마다(`notifStart`) 확인, 토스트 + 선택적 브라우저 Notification.
- **반응형**: 데스크톱(>1024) / 태블릿(641–1024: 대시보드 LNB 200px, 카드 2열, 달력·최근 세로; 편집 화면은 881–1199에서 문서 폭을 화면에 맞추고 `.dochead` 한 줄 하나) / 모바일(≤640: 대시보드 LNB가 위쪽 띠 — 로고·새 회의·`#lnbMore` 메뉴 토글(`.lnb.open`), 프로젝트 칩 가로 스크롤). A4 고정 폭(`210mm`)은 1200px 이상에서만.
...
- **프로젝트**(회의록 양식 옆 선택 + 관리 창): 회의록을 프로젝트로 묶는다(`S.meta.project` = 프로젝트 id, 없어도 됨). 프로젝트 목록은 localStorage `meetnote.projects`에 두고 로그인 시 Firestore `users/{uid}/projects/{id}`(json 필드)에 동기화한다(updatedAt 최신 우선, 규칙에 경로가 없으면 403 → 브라우저에만). 목록 DB(meetings)에도 `project`·`projectName`을 적어 불러오기 창에서 모든 프로젝트/프로젝트 없음/특정 프로젝트로 거르고, 카드의 선택 상자로 바로 지정·변경한다(목록 DB가 파일보다 우선: 클라우드에서 열 때 적용).
  ...
  - **공유**: 프로젝트 관리의 구성원 칸에서 Google 이메일로 초대(`p.members`, 소유자 `p.owner`/`ownerEmail`). 동기화 위치는 최상위 Firestore `projects/{id}`(memberEmails 배열, `fsQuery` array-contains로 내 것을 당김; 옛 `users/{uid}/projects`는 첫 로그인 때 자동 이전). 프로젝트에 묶인 회의록을 클라우드 저장하면 `projects/{id}/meetings/{mid}`에 기록하고 올린 사람의 Drive 파일을 구성원에게 편집 권한으로 공유(`shareFileWith`, 알림 메일 없음). 불러오기 목록은 내 목록 + 공유 프로젝트 목록을 합친다(`sharedRows`, 공유 배지·올린 사람 표시, 삭제·프로젝트 변경은 올린 사람만). 남의 공유 파일에 쓸 권한이 없으면 내 Drive에 새 파일로 저장하고 안내한다. 소유자만 초대·내보내기·이름 변경·삭제, 구성원은 나가기. 구성원 칸은 초대 입력란 아래 세로 목록(`renderProjMembers`: 마스터 → 초대 담당자 → 구성원, 8명 넘으면 `#projMemFilter` 검색). **보강 동기화** `projMeetSync(rows)`: 대시보드를 열 때(`cloudOpenList`)와 초대할 때 내 회의록 중 공유 프로젝트에 묶인 것이 `projects/{id}/meetings`에 빠졌거나 오래됐으면 다시 기록하고, Drive 토큰이 있으면 구성원에게 파일을 공유한다(어디에 공유했는지는 `meetnote.sharedFiles`에 기억). 공유 전에 저장한 회의록이 구성원에게 안 보이던 문제의 해결책. 구성원 쪽에서 기록이 하나도 없으면 목록에 이유를 안내한다. **함께 열었을 때**: 프로젝트 회의록을 열고 있는 동안 `projects/{id}/meetings/{mid}`에 `editing`·`editingAt`을 30초마다 남기고(`presStart/presStop`, 90초 지나면 무시) 목록에 "✎ 편집 중"으로 표시한다. 다른 사람이 편집 중인 회의록은 읽기 전용(`setReadOnly`: 편집기·docsheet 입력 잠금, 저장 막음)으로 열리고 `#lockTake`로 이어받는다. 클라우드 저장 직전에 Drive `modifiedTime`을 연 시점(`S.cloud.modifiedTime`)과 비교해 그 사이 저장이 있었으면 `askChoice`로 최신 불러오기 / 새 회의록으로 저장(fork: 새 id) / 덮어쓰기를 고르게 한다. 구성원이 올린 사람의 파일에 되쓰면 기록의 올린 사람은 유지하고 `editedBy`를 남긴다(`sharedRows`가 내 행에 더 최근 기록을 반영).
