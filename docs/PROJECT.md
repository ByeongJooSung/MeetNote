...
- **AI로 발언자 추정**(네 번째 방식, 실험, `diarizeByLLM`): 녹음 없이 전사 글만 현재 AI 엔진(`aiComplete`)에 보내 60줄씩 줄마다 화자를 추정하고(JSON), 참석자 목록을 근거로 화자에 이름을 붙인다. 이미 화자가 나뉘어 있으면 이름 붙이기만 한다. 목소리를 듣지 않으므로 정확도가 낮다는 점을 화면에 명시. 휴대폰에서는 브라우저 내장 '정확' 모델을 균형으로 강제(`resolveTier`).
